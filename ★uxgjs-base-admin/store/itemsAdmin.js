...
export const actions = {
  ...
  uploadSkinFile ({ commit }, {id, skinFile}) {
    return new Promise((resolve, reject) => {
      ...
      ref.put(skinFile).then((snapshot) => {
        snapshot.ref.getDownloadURL().then((downloadURL) => {
            DB.collection('items').doc(id).get().then(function(doc) {
              var fileName = snapshot.ref.name;
              var currentSkinFile= {};
              if (doc.data().skinFile) {
                  currentSkinFile = doc.data().skinFile;
                  currentSkinFile[fileName] = downloadURL;
              } else {
                currentSkinFile[fileName] = downloadURL;
              }
              return DB.collection('items').doc(id).update( {skinFile: currentSkinFile} );
  ...
  skinFileDelete({ commit }, {id, skinFileKey}) {
    const ref = STORAGE.ref().child(`items/${id}/${skinFileKey}`)
    ref.delete().then(function() {
      DB.collection('items').doc(id).get().then(function(doc) {
        var currentSkinFile= {};
        if (doc.data().skinFile[skinFileKey]) {
            currentSkinFile = doc.data().skinFile;
            delete currentSkinFile[skinFileKey] ;
            return DB.collection('items').doc(id).update( {skinFile: currentSkinFile} );
        }
    }).catch(function(error) {
        console.log("Error getting document:", error);
